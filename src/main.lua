...
local im_module_warning = function(out, envvar) return out:write(F[[
...
local ibus_module_warning = function(out, envvar) return out:write(F[[
...
local generated_header = function(out, progname) return out:write(F[[
...
local function xcomposer(args)
  ...
  for _, envvar in ipairs({"GTK_IM_MODULE", "QT_IM_MODULE"}) do
    ...
    if not value then
      im_module_warning(io.stderr, envvar)
    end
    if value == "ibus" then
      ibus_module_warning(io.stderr, envvar)
    end
  end
  ...
  generated_header(outfile, XCOMPOSER_NAME)
