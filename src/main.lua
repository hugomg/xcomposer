...
local im_module_warning = function(envvar) return [[
...
local ibus_module_warning = function(envvar) return [[
...
local generated_header = function(progname) return [[
...
local function xcomposer(args)
  ...
  for _, envvar in ipairs({"GTK_IM_MODULE", "QT_IM_MODULE"}) do
    ...
    if not value then
      io.stderr:write(im_module_warning(envvar))
    end
    if value == "ibus" then
      io.stderr:write(ibus_module_warning(envvar))
    end
  end
  ...
  outfile:write(generated_header(XCOMPOSER_NAME))
